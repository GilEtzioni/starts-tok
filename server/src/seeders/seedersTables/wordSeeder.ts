...
import { CourseLangauge, EnglishLevel, HebrewLevel, Word } from "../../types/seedersType";
...
{ hebrew: "הורים", english: "parents", german: "Eltern", italian: "genitori", spanish: "padres", french: "parents", courseNameEnglish: "Family Members", courseOrder: 3 },
  ...
  const getCourseLevel = (courseOrder: number) => {
    if (courseOrder <= 25) {
      return { hebrewLevel: HebrewLevel.A1, englishLevel: EnglishLevel.A1};
    } else if (courseOrder <= 50) {
      return { hebrewLevel: HebrewLevel.A2, englishLevel: EnglishLevel.A2 };
    } else if (courseOrder <= 75) {
      return { hebrewLevel: HebrewLevel.B1, englishLevel: EnglishLevel.B1 };
    } else if (courseOrder <= 100) {
      return { hebrewLevel: HebrewLevel.B2, englishLevel: EnglishLevel.B2 };
    } else if (courseOrder <= 125) {
      return { hebrewLevel: HebrewLevel.C1, englishLevel: EnglishLevel.C1 };
    } else if (courseOrder <= 150) {
      return { hebrewLevel: HebrewLevel.C2, englishLevel: EnglishLevel.C2 };
    }
  }
...
const wordData = words.flatMap((word) => {
  ...
  return [
    { ...commonData, language: CourseLangauge.English, foreignWord: word.english, },
    { ...commonData, language: CourseLangauge.German, foreignWord: word.german },
    { ...commonData, language: CourseLangauge.French, foreignWord: word.french },
    { ...commonData, language: CourseLangauge.Italian, foreignWord: word.italian },
    { ...commonData, language: CourseLangauge.Spanish, foreignWord: word.spanish },
  ];
});
