...
interface WordsGridProps {
  ...
  allWords: Array<WordsType>;
}

const LettersGrid: React.FC<WordsGridProps> = ({ correctAnswer, gridAnswer, setGridAnswer, gridLetters, setGridLetters, allWords }) => {
    
    const clicksCounter = useSelector((state: RootState) => state.wordel.clicksCounter);
    const dispatch = useDispatch();

    function handleLetterClick(letter: string) {
        let columnIndex = gridAnswer[clicksCounter]?.findIndex((item) => item === null) ?? -1;
        ...
        if (columnIndex !== correctAnswer.length) {
            ...
            const newLetter: wordleType = { letterColor: color, letter, isInGame: false };
