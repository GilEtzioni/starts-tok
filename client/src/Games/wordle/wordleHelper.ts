...
export const randomWordsArray = (words: Array<WordsType>): Array<WordsType>  => {
    const gridSizeArray = [4, 5, 6, 7];
    const gridSizeIndex = Math.floor(Math.random() * gridSizeArray?.length + 1);
    const gridSize = gridSizeArray[gridSizeIndex];

    const suffeledArray = shuffleAllWords(words);
    const result: Array<WordsType>  = [];

    suffeledArray.forEach((item) => {
        if (item.GermanWord?.length === gridSize) {
            result.push(item);
        }
    })
    return result;
...
export const getRandomWord = (words: Array<WordsType>): Array<wordleType> => {

    const index = Math.floor(Math.random() * words?.length);
    const sentence = words[index]?.GermanWord;

    const result: Array<wordleType> = [];
...
export const createGameGrid = (correctAnswer: Array<wordleType>) => {
...
export const createLettersGrid = (): Array<{letter: string, letterColor: letterColor}> => {
    ...
    const lettersArray = ['a', 'ä', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'ö', 'p', 'q', 'r', 's', 'ß', 't', 'u', 'ü', 'v', 'w', 'x', 'y', 'z'];
...
export const getLetterColor = (letter: string, columnIndex: number, correctAnswerArray: Array<wordleType>): letterColor => {
    ...
    const isLetterInArray: boolean = correctAnswerArray.some((item) => item?.letter === letter);
    if (isLetterInArray) {
