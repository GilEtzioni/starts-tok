...
import { setRunning, addOneOrder, resetClicks } from "../dataLessons/LessonsSlice";

interface ErrorMessageProps {
    onClick?: () => void;
}

const ErrorMessage: React.FC<ErrorMessageProps> = ({ onClick }) => {
    ...
    const rightAnswer = (order: number, answer: string) => {
        if (order === 1 || order === 4) {
            return (
                ...
                style={{ top: '110px'}}
                    bordered={false}
                    className="bg-rose-200 text-white text-center flex items-center justify-center w-72 h-24 mx-auto rounded-lg relative shadow-md"
                >
            ...
            return (
                <Card
                    ...
                    onClick={handleClick}
                    bordered={false}
                    className="bg-rose-200 text-white text-center flex items-center justify-center w-72 h-36 mx-auto rounded-lg relative shadow-md"
                >
                    <div className="absolute top-2 right-2 flex flex-col items-end text-lg">
                        <div className="flex items-center">
                            <span className="rtl mr-1 text-red-600">!טעות</span>
